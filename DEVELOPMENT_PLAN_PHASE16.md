...
5. **`tests/fixtures/golden/`**：`sample_purchase.json`、`sample_service.json`、**`sample_markdown_lines.json`**（各含脱敏 `text` + `expect` 上下界；行级样本可含 `input_parse_mode` / `markdown_line_count_min`）；`tests/test_golden_regression.py`（默认 `LLM_MODE=stub`，范围断言）。
