...
- **工作流可观测**：`summary.pending_object_field_library`（§4.4）、`summary.source_library_meta`、**`summary.field_extraction_task_counts`**（§5.1 粗/精任务数）；**dry-run** 另含完整 `source_library` 与 **`field_extraction_tasks`** `{mode_1, mode_23}`（第十一阶段）
